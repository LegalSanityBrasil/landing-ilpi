...
├── index.html
├── assets/
│   ├── icon.png   (usado no cabeçalho, rodapé e no selo — obrigatório)
│   └── logo.png   (logo completa, mantida como referência/backup)
└── README.md
```

**Importante:** o site agora usa `assets/icon.png` (só o símbolo do mapa, sem o texto embutido) no cabeçalho, rodapé e selo giratório, para não duplicar visualmente o nome "Legal Sanity Brasil" que já aparece em texto ao lado. Os dois arquivos de imagem precisam estar na pasta `assets/`.
